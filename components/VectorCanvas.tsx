...
const VectorCanvas: React.FC<VectorCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    // --- GRID ---
    if (gridMode !== 'none') {
      ctx.strokeStyle = gridMode === 'light' 
    ...
    ctx.font = '14px Inter, sans-serif'; // Larger axis labels
    ...
    const drawArrow = (x: number, y: number, color: string, label: string, isSelected: boolean, isGhost = false) => {
        ...
        ctx.fillStyle = color;
        ctx.lineWidth = isSelected ? 4 : 2.5; // Thicker lines
        ...
        // Label
        if (label) {
            ctx.font = isSelected ? 'bold 16px Inter, sans-serif' : '14px Inter, sans-serif'; // Larger font
    ...
    const drawAngleArc = (x: number, y: number, radius = 25) => {
        ...
        if (true) {
          ...
          ctx.font = '500 12px Inter, sans-serif'; // Larger angle font
          ctx.fillStyle = '#64748B';
          const text = `${Math.abs(diff).toFixed(0)}°`;
          const tm = ctx.measureText(text);
          
          ctx.lineWidth = 3;
          ctx.strokeStyle = isDarkMode ? '#1e293b' : '#FFFFFF'; // Contrast stroke for text
  ...
  }, [vectors, resultant, missingVector, showDashedLines, gridMode, selectedVectorId, rotationAngle, offset, readOnly, scale, isDarkMode]);
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden bg-white/50 dark:bg-slate-800/50">
       <canvas
         ref={canvasRef}
