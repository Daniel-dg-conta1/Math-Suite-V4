...
interface VectoraAppProps {
  isDarkMode?: boolean;
}

const Tooltip = ({text}: {text: string}) => (
  <div className="group relative inline-block ml-1 align-middle">
    <div className="cursor-help text-gray-400 hover:text-purple-500 transition-colors">
      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    </div>
    <div className="invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-slate-800 text-white text-[10px] p-2 rounded shadow-lg z-50 text-center pointer-events-none">
      {text}
...
const VectoraApp: React.FC<VectoraAppProps> = ({ isDarkMode }) => {
  // --- STATE ---
  ...
  // Updated state to include input strings for proper editing support
  const [desiredResultant, setDesiredResultant] = useState<{
    x: number; y: number; mag: number; ang: number;
    inputX: string; inputY: string; inputMag: string; inputAng: string;
  }>({ 
    x: 5, y: 5, mag: 7.07, ang: 45,
    inputX: '5.00', inputY: '5.00', inputMag: '7.07', inputAng: '45.00'
  });
  ...
  const handleScrollZoom = useCallback((deltaY: number) => {
    setCanvasScale(prev => {
        // Delta < 0 usually means scroll UP (zoom in), Delta > 0 scroll DOWN (zoom out)
        const direction = deltaY > 0 ? -1 : 1;
        const step = 2; 
        const next = prev + direction * step;
        // Updated min to 5 to match autoscale limits
        return Math.min(Math.max(next, 5), 100);
    });
  }, []);
  ...
  const handleAutoscale = () => {
    ...
    // Collect points: origin + vector heads
    const points = [{x: 0, y: 0}];
    vectors.forEach(v => points.push({x: v.x, y: v.y}));
    
    // Include resultant/missing logic
    points.push({x: displayedResultant.x, y: displayedResultant.y});
    if (missingMode && missingVectorCalc) {
       points.push({x: missingVectorCalc.x, y: missingVectorCalc.y});
    }
    ...
    // Prevent huge scale if vectors are near zero
    if (maxX < 1) maxX = 1;
    if (maxY < 1) maxY = 1;

    // Margin in pixels (for arrow heads, labels, and padding)
    const margin = 60; 
    
    // Calculate scale to fit in half-width/height (since origin is centered)
    const scaleX = (clientWidth / 2 - margin) / maxX;
    const scaleY = (clientHeight / 2 - margin) / maxY;
    
    let optimal = Math.min(scaleX, scaleY);
    
    // Clamp constraints: Min 5, Max 100 (keep standard max)
    optimal = Math.max(5, Math.min(optimal, 100));
    
    setCanvasScale(optimal);
  };
  ...
  const getSummaryData = () => {
     const rows = [];
     // Resultant
     const rMagVal = getMagnitude(displayedResultant.x, displayedResultant.y);
     const rAngVal = getAngle(displayedResultant.x, displayedResultant.y);
     rows.push({
        label: missingMode ? 'Rd (Alvo)' : 'R (Soma)',
        x: formatNumber(displayedResultant.x),
        y: formatNumber(displayedResultant.y),
        mag: formatNumber(rMagVal),
        ang: formatNumber(rAngVal)
     });

     // Missing
     if (missingMode && missingVectorCalc) {
        const mMag = getMagnitude(missingVectorCalc.x, missingVectorCalc.y);
        const mAng = getAngle(missingVectorCalc.x, missingVectorCalc.y);
        rows.push({
           label: 'Vf (Faltante)',
           x: formatNumber(missingVectorCalc.x),
           y: formatNumber(missingVectorCalc.y),
           mag: formatNumber(mMag),
           ang: formatNumber(mAng)
        });
     }
     ...
     vectors.forEach(v => {
        ...
        rows.push({
           ...
           x: formatNumber(v.x),
           y: formatNumber(v.y),
           mag: formatNumber(mag),
           ang: formatNumber(ang)
        });
     });
     return rows;
  };
  ...
  const handleCopySummary = () => {
     ...
     const header = "Vetor\tx\ty\t|V|\tθ°";
     const rows = data.map(d => `${d.label}\t${d.x}\t${d.y}\t${d.mag}\t${d.ang}`);
  ...
  const handleExportCSV = () => {
     ...
     link.href = url;
     link.setAttribute('download', 'Vectora_Resumo.csv');
     document.body.appendChild(link);
     link.click();
     document.body.removeChild(link);
  };

  const formatFloatInput = (n: number) => n.toFixed(2);

  const handleDesiredChange = (key: 'x'|'y'|'mag'|'ang', val: string) => {
     const newR = { ...desiredResultant };
     
     // Update raw input string
     if (key === 'x') newR.inputX = val;
     if (key === 'y') newR.inputY = val;
     if (key === 'mag') newR.inputMag = val;
     if (key === 'ang') newR.inputAng = val;
     
     // Parse safely (handle empty, minus sign, comma)
     const normalized = val.replace(',', '.');
     const numVal = parseFloat(normalized);
     const safeVal = (isNaN(numVal) || val === '' || val === '-') ? 0 : numVal;

     if (key === 'x') {
        newR.x = safeVal;
        newR.mag = getMagnitude(newR.x, newR.y);
        newR.ang = getAngle(newR.x, newR.y);
        newR.inputMag = formatFloatInput(newR.mag);
        newR.inputAng = formatFloatInput(newR.ang);
     } else if (key === 'y') {
        newR.y = safeVal;
        newR.mag = getMagnitude(newR.x, newR.y);
        newR.ang = getAngle(newR.x, newR.y);
        ...
        newR.inputX = formatFloatInput(newR.x);
        newR.inputY = formatFloatInput(newR.y);
     } else if (key === 'ang') {
        newR.ang = safeVal;
        const rad = newR.ang * DEG_TO_RAD;
        newR.x = newR.mag * Math.cos(rad);
  ...
  const PreviewModal = () => (
    <div className="fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-6">
      <div className="bg-white dark:bg-slate-900 w-full max-w-5xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-gray-200 dark:border-slate-800">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50">
          <div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">Pré-visualização da Lista</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">Confira os {generatedExercises.length} exercícios gerados antes de imprimir.</p>
          </div>
          <button onClick={() => setShowPreview(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-700 p-2 rounded-full transition">✕</button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50/30 dark:bg-slate-950 custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {generatedExercises.map((ex, idx) => {
              ...
              return (
                <div key={idx} className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm p-4 hover:shadow-md transition">
                  <div className="flex justify-between items-center mb-3">
                    <div className="flex items-center gap-2">
                       <span className="font-bold text-gray-700 dark:text-gray-200">Questão {idx + 1}</span>
                       <span className="text-xs bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300 px-2 py-1 rounded">{ex.type}</span>
                    </div>
                    <button onClick={() => regenerateExercise(idx)} className="text-[10px] text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 px-2 py-1 rounded border border-red-100 dark:border-red-900 transition font-bold">Excluir e Refazer</button>
                  </div>
                  <div className="h-48 w-full bg-gray-50 dark:bg-slate-800 rounded-lg overflow-hidden relative border border-gray-100 dark:border-slate-700">
                    <VectorCanvas vectors={previewVectors} resultant={previewRes} missingVector={previewMissing} showDashedLines={false} gridMode="none" selectedVectorId={null} rotationAngle={0} readOnly={true} isDarkMode={isDarkMode} />
                  </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-end gap-3">
           <button onClick={() => setShowPreview(false)} className="px-4 py-2 text-gray-600 dark:text-gray-300 font-medium hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition">Voltar</button>
           <button onClick={() => generatePDF(generatedExercises, false, pdfMode, itemsPerPage)} className="px-4 py-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-200 font-bold rounded-lg hover:border-blue-400 hover:text-blue-600 transition flex items-center gap-2">📄 Baixar PDF Aluno</button>
           <button onClick={() => generatePDF(generatedExercises, true, pdfMode, itemsPerPage)} className="px-4 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-lg shadow-blue-200 dark:shadow-none transition flex items-center gap-2">🎓 Baixar Gabarito</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex-1 p-6 grid grid-cols-12 gap-6 min-h-0 overflow-hidden">
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar h-full">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 p-5">
            <div className="flex gap-2 mb-2">
               <button onClick={() => setMissingMode(false)} className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${!missingMode ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 shadow-sm' : 'bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-gray-200 dark:hover:bg-slate-700'}`}>Resultante</button>
               <button onClick={() => setMissingMode(true)} className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${missingMode ? 'bg-teal-100 dark:bg-teal-900 text-teal-700 dark:text-teal-200 shadow-sm' : 'bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-gray-200 dark:hover:bg-slate-700'}`}>Alvo / Faltante</button>
            </div>
            {missingMode && (
               <div className="animate-in fade-in slide-in-from-top-2 duration-200 bg-teal-50/50 dark:bg-teal-900/20 p-2 rounded-lg border border-teal-100 dark:border-teal-800">
                  <div className="flex justify-between items-center mb-1">
                     <span className="text-[10px] font-bold text-teal-800 dark:text-teal-300 uppercase">Definir Resultante Alvo (Rd)</span>
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 flex flex-col">
             <div className="p-4 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50 rounded-t-xl">
               <h2 className="font-semibold text-gray-700 dark:text-gray-200">↗ Vetores</h2>
               <button onClick={addVector} className="text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 px-2 py-1 rounded font-bold hover:bg-blue-100 dark:hover:bg-blue-900/50 transition">+ Novo</button>
             </div>
             <div className="p-5 space-y-4">
               {vectors.map(v => (
                 <div key={v.id} className={`border rounded-xl p-5 transition-all ${selectedVectorId === v.id ? 'border-blue-400 ring-1 ring-blue-100 dark:ring-blue-900' : 'border-gray-200 dark:border-slate-700'}`}>
                    <div className="flex justify-between items-center mb-2" onClick={() => setSelectedVectorId(v.id)}>
                      <div className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="font-bold text-sm text-gray-800 dark:text-gray-200">{v.label}</span>
                      </div>
                    ...
                    <div className="flex gap-1 mt-2">
                       <button onClick={() => updateVector(v.id, {inputType: 'cartesian'})} className={`flex-1 text-[10px] py-1 rounded ${v.inputType === 'cartesian' ? 'bg-gray-200 dark:bg-slate-600 font-bold dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>(x,y)</button>
                       <button onClick={() => updateVector(v.id, {inputType: 'angle'})} className={`flex-1 text-[10px] py-1 rounded ${v.inputType === 'angle' ? 'bg-gray-200 dark:bg-slate-600 font-bold dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>(r,θ)</button>
                       <button onClick={() => updateVector(v.id, {inputType: 'triangle'})} className={`flex-1 text-[10px] py-1 rounded ${v.inputType === 'triangle' ? 'bg-gray-200 dark:bg-slate-600 font-bold dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>Tri</button>
                    </div>
                 </div>
               ))}
             </div>
          ...
          {/* SUMMARY TABLE WITH TOOLS */}
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 flex flex-col p-5 space-y-2">
             <div className="flex justify-between items-center mb-1">
                <button 
                  onClick={() => setIsSummaryExpanded(!isSummaryExpanded)}
                  className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase flex items-center gap-2 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"
                >
                   <span className="text-[10px]">{isSummaryExpanded ? '▼' : '▶'}</span> Resumo de Dados
                </button>
               ...
               <div className="overflow-x-auto animate-in fade-in slide-in-from-top-1 duration-200">
                 <table className="w-full text-xs text-left">
                    ...
                    <tbody className="divide-y divide-gray-50 dark:divide-slate-800 text-gray-600 dark:text-gray-300">
                      {/* Linha da Resultante */}
                      <tr className="font-bold text-gray-800 dark:text-white bg-gray-50 dark:bg-slate-800/50">
                      ...
                      {missingMode && missingVectorCalc && (
                         <tr className="font-bold text-teal-700 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20">
          ...
          {/* ROTATION & TABLE */}
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 flex flex-col p-5">
             <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-3">Rotação de Eixos (α)</h3>
             <div className="flex items-center gap-2 mb-4">
                <input 
                  type="number" 
                  value={rotationAngle} 
                  onChange={(e) => setRotationAngle(parseFloat(e.target.value)||0)}
                  className="p-2 border rounded w-24 bg-gray-50 dark:bg-slate-800 dark:text-white dark:border-slate-600 text-sm"
                />
                <span className="text-sm text-gray-600 dark:text-gray-400">graus</span>
             </div>
             
             <h4 className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 mb-2 border-b border-indigo-100 dark:border-indigo-900 pb-1">Componentes (x', y')</h4>
             <div className="overflow-x-auto">
               <table className="w-full text-xs text-left">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-slate-800 text-gray-600 dark:text-gray-300">
                    <tr className="font-bold text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20">
        ...
        <div className="col-span-12 lg:col-span-6 flex flex-col h-full relative" ref={canvasContainerRef}>
           <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-200 dark:border-slate-700 flex flex-col overflow-hidden w-9 items-center">
                 <button onClick={() => setCanvasScale(s => Math.min(s + 5, 100))} className="h-9 w-full hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-600 dark:text-gray-300 border-b border-gray-100 dark:border-slate-700 flex items-center justify-center transition-colors" title="Aumentar Zoom">
                 ...
                 <button 
                    ...
                    className="h-9 w-full hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-600 dark:text-gray-300 border-b border-gray-100 dark:border-slate-700 flex items-center justify-center transition-colors" 
                    title="Ajuste Automático (Autoscale)"
                 >
                 ...
                 <button onClick={() => setCanvasScale(s => Math.max(s - 5, 5))} className="h-9 w-full hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-600 dark:text-gray-300 flex items-center justify-center transition-colors" title="Diminuir Zoom">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
           ...
           <VectorCanvas 
              ...
              onZoom={handleScrollZoom}
           />
           
           <div className="absolute bottom-6 left-6 right-6 bg-white/90 dark:bg-slate-900/90 backdrop-blur border border-gray-200 dark:border-slate-700 p-4 rounded-xl shadow-lg flex justify-between items-center pointer-events-none">
               <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">{missingMode ? 'Alvo (Rd)' : 'Resultante (R)'}</div>
               ...
               <div className="text-right">
                  <div className="text-sm font-bold text-gray-700 dark:text-gray-300">|{missingMode ? 'Rd' : 'R'}| = {formatNumber(rMag)}</div>
                  <div className="text-sm font-bold text-gray-700 dark:text-gray-300">θ = {formatNumber(rAng)}°</div>
               </div>
           </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-6 pl-2 h-full overflow-y-auto custom-scrollbar">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 flex flex-col">
             ...
             <div className="p-6 space-y-6">
                <div>
                   <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 block">Quantidade: {numQuestions}</label>
                ...
                <div>
                   <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 block">Tipos de Questão</label>
                   <div className="grid grid-cols-2 gap-2">
                      {['2','3','4','missing'].map(t => (
                        <label key={t} className={`p-2 border rounded-md text-xs font-medium text-center cursor-pointer transition ${selectedTypes.includes(t) ? 'bg-purple-100 dark:bg-purple-900/50 border-purple-300 dark:border-purple-700 text-purple-800 dark:text-purple-200' : 'hover:bg-gray-50 dark:hover:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-400'}`}>
                ...
                <div className="space-y-3 pt-6 border-t border-gray-100 dark:border-slate-800">
                    <div>
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1">
                          ...
                          <Tooltip text="A diferença mínima de ângulo entre vetores gerados para evitar sobreposição visual." />
                        </label>
                        <input type="number" value={minSep} onChange={e => setMinSep(parseInt(e.target.value)||15)} className="w-full p-2 border rounded bg-gray-50 dark:bg-slate-800 dark:text-white dark:border-slate-700 text-sm" />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            <label className="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1">
                              Mínimo módulo
                              <Tooltip text="O menor valor possível para a magnitude de um vetor gerado." />
                            </label>
                            <input type="number" value={minGenMag} onChange={e => setMinGenMag(parseInt(e.target.value)||5)} className="w-full p-2 border rounded bg-gray-50 dark:bg-slate-800 dark:text-white dark:border-slate-700 text-sm" />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1">
                              Máximo módulo
                              <Tooltip text="O maior valor possível para a magnitude de um vetor gerado." />
                            </label>
                            <input type="number" value={maxGenMag} onChange={e => setMaxGenMag(parseInt(e.target.value)||10)} className="w-full p-2 border rounded bg-gray-50 dark:bg-slate-800 dark:text-white dark:border-slate-700 text-sm" />
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-6 border-t border-gray-100 dark:border-slate-800">
                    ...
                    {pdfMode === 'simple' && (
                        <div className="flex items-center gap-2 animate-in fade-in">
                            <label className="text-xs text-gray-600 dark:text-gray-400">Exercícios/pág:</label>
                            <input type="number" min="1" max="8" value={itemsPerPage} onChange={(e) => setItemsPerPage(parseInt(e.target.value)||4)} className="w-16 p-1 border rounded text-xs dark:bg-slate-800 dark:text-white dark:border-slate-700" />
                        </div>
                    )}
                </div>
                ...
                <div className="pt-6 border-t border-gray-100 dark:border-slate-800">
                   <button 
                     ...
                     className="w-full py-3 bg-purple-600 text-white font-bold rounded-lg shadow-lg shadow-purple-200 dark:shadow-none hover:bg-purple-700 hover:scale-[1.02] transition-all disabled:opacity-50 disabled:scale-100"
                   >
                     Gerar Lista & Preview
                   </button>
                   
                   {generatedExercises.length > 0 && (
                      <button onClick={() => setShowPreview(true)} className="w-full mt-3 py-2 text-purple-600 dark:text-purple-400 text-sm font-bold hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded-lg transition">
